...
int8_t Message::parseOptions(uint8_t *packetBuffer, uint8_t messageEnd){
      ...
      this->options = (Option*) calloc(0, sizeof(Option));
...
uint8_t Message::parsePayload(uint8_t *packetBuffer, int16_t firstByte, uint8_t messageEnd){
    // this->payload = (uint8_t*) malloc(0)
    //if(no error code occured)
    if(firstByte > 0){
        uint16_t length = messageEnd - firstByte;
        //allocates memory to an array, based on payload length
        this->payload = (uint8_t*) calloc(length, sizeof(uint8_t));
        // Serial.print(F("Alokuje tyle payloadu: ")); Serial.println(sizeof(uint8_t)*length);
...
//error catching allows us to return adequate payload in response
...
//assemblers - they are responsible for creating response messages
//typically, token and message ID are copied from a message that we are handling
...
//with this function, we can prepare some options to be assembled
//if we want to add multiple options, we have to add them in increasing order
void Message::addOption(uint16_t number, const char *val){

    //reads value from flash memory. We have most of those values stored in
    //  utils/Strings.h file
...
//it is called to assemble every option that we have added with addOption() function
...
uint16_t length;
    ...
    for(uint8_t i = 0; i < optionCounter; i++){
        ...
        //makes necessary calculations to track where are we now in a buffer
        ...
        //as soon as we assembled options and calculated position of next byte,
        //we deallocate memory in option object
...
//writes payload (stored in utils/String.h file) into a buffer
void Message::assemblePayload(uint8_t *packetBuffer, const char *_payload, uint16_t firstByte){
    ...
    for(uint16_t i = 0; i < strlen(_payload); i++){
        packetBuffer[firstByte + i] = _payload[i];
    }
}

uint16_t Message::assemble(uint8_t *packetBuffer, uint8_t typ, uint8_t cod, const char *_payload){

    this->clearPacketBuffer(packetBuffer);
    this->assembleHeader(packetBuffer, typ, cod);
    this->assembleToken(packetBuffer);
    uint16_t firstByte =
    this->assembleOptions(packetBuffer);
    this->assemblePayload(packetBuffer, _payload, firstByte);

    uint16_t payloadLength = strlen(_payload);

    return firstByte + payloadLength;
}

uint16_t Message::assemble(uint8_t *packetBuffer, uint8_t typ, uint8_t tkl, uint8_t cod, uint64_t tok, const char *_payload){

    this->clearPacketBuffer(packetBuffer);
    this->header.setTokenLength(tkl);
    this->assembleHeader(packetBuffer, typ, cod);
    this->setToken(tok);
    this->assembleToken(packetBuffer);
    uint16_t firstByte =
...
//used to conveniently pass a response code as an argument
...
void Message::clearPacketBuffer(uint8_t *packetBuffer){
...
void Message::freeMemory(uint8_t parsingResult){
    ...
    if(parsingResult == 1){
        Serial.println(F("Zwalniam pamiec payloadu"));
        free(this->payload);
        fix_malloc_bug();
        this->payload = NULL;
    }
